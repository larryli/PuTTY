                      ...
                      int protocol, int protcfginfo)
{
    ...
    ctrl_checkbox(s, "即使在UTF-8模式下也启用VT100线条图",'8',
    ...
    if (backend_vt_from_proto(PROT_SSH) &&
        ...
        if (protcfginfo != 1 && protcfginfo != -1) {
            ...
            s = ctrl_getset(b, "连接/SSH/密钥", "main",
                            "密钥验证算法选项");
            c = ctrl_draglist(s, "算法选择策略(S)：", 's',
                              HELPCTX(ssh_kexlist),
                              kexlist_handler, P(NULL));
            c->listbox.height = KEX_MAX;
#ifndef NO_GSSAPI
            ctrl_checkbox(s, "尝试使用 GSSAPI 密钥验证",
        ...
        if (!midsession) {
            ...
            ctrl_checkbox(s, "尝试使用 GSSAPI 认证 (仅限SSH-2)",
                          't', HELPCTX(ssh_gssapi),
                          conf_checkbox_handler,
                          I(CONF_try_gssapi_auth));

            ctrl_checkbox(s, "尝试使用 GSSAPI 密钥验证 (仅限SSH-2)",
                          'k', HELPCTX(ssh_gssapi),
                          conf_checkbox_handler,
                          I(CONF_try_gssapi_kex));
