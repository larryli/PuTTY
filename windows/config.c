                          ...
                          bool midsession, int protocol)
{
    ...
    ctrl_checkbox(s, "Control-Alt 与 AltGr 不同功能(D)", 'd',
    ...
    ctrl_filesel(s, "播放响铃的自定义声音文件：", NO_SHORTCUT,
                 FILTER_WAVE_FILES, false, "选择响铃声音文件",
                 HELPCTX(bell_style),
     ...
     */
    ctrl_radiobuttons(s, "响铃时任务栏/标题栏动作(I)：", 'i', 3,
                      HELPCTX(bell_taskbar),
                      conf_radiobutton_handler,
                      I(CONF_beep_ind),
                      "禁止", I(B_IND_DISABLED),
                      "闪烁", I(B_IND_FLASH),
                      "保持", I(B_IND_STEADY), NULL);
    ...
    ctrl_radiobuttons(s, "字体质量(Q)：", 'q', 2,
    ...
    ctrl_checkbox(s, "Caps Lock 大小写切换 Cyrillic", 's',
    ...
    s = ctrl_getset(b, "窗口/转换", "trans",
                    "接收数据的字符集转换");
    ctrl_text(s, "(支持手工输入列表中没有列出的 Windows 代码页 "
              "比如 CP936)",
     ...
     * characters.
     */
    str = dupprintf("调整 %s 重绘字符行显示", appname);
        ...
        for (i = 0; i < s->ncontrols; i++) {
            ...
            if (c->generic.type == CTRL_RADIO &&
                ...
                c->radio.buttons[c->radio.nbuttons-3] =
                    dupstr("XWindows 编码字体");
                c->radio.buttons[c->radio.nbuttons-2] =
                    dupstr("使用 ANSI 和 OEM 模式字体(B)");
                c->radio.buttons[c->radio.nbuttons-1] =
                    dupstr("只使用 OEM 模式字体");
                c->radio.buttondata =
                    sresize(c->radio.buttondata, c->radio.nbuttons, intorptr);
    ...
    s = ctrl_getset(b, "窗口/选择/复制", "format",
                    "复制字符格式化");
    ctrl_checkbox(s, "RTF 也使用纯文本格式复制到剪贴板", 'f',
                  HELPCTX(copy_rtf),
    ...
    if (!midsession || !resize_forbidden) {
        ...
        ctrl_radiobuttons(s, "当窗口大小改变时(Z)：", 'z', 1,
                          HELPCTX(window_resize),
                          conf_radiobutton_handler,
                          I(CONF_resize_action),
                          "修改行列数", I(RESIZE_TERM),
                          "修改字体大小", I(RESIZE_FONT),
                          "只在最大化时修改字体大小", I(RESIZE_EITHER),
    ...
    if (!midsession) {
        ...
        for (i = 0; i < s->ncontrols; i++) {
            ...
            if (c->generic.type == CTRL_EDITBOX &&
                ...
                c->generic.label = dupstr("Telnet 命令或本地"
                                          "代理命令");
                break;
            }
        }
    ...
    if (!midsession && backend_vt_from_proto(PROT_SSH)) {
        ...
        ctrl_filesel(s, "用于本地显示的 X 权限文件(T)", 't',
                     NULL, false, "选择 X 权限文件",
